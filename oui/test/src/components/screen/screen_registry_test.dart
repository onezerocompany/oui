...
void main() {
  group('ScreenRegistryEntry', () {
    ...
    test('should match dynamic path segments', () {
      ...
      final path = Path([PathSegment.argument('userId')]);
      final entry = ScreenRegistryEntry(screen, path);
  ...
  group('ScreenRegistry', () {
          ...
          testScreen(
            ...
            children: [
              ...
              testScreen(
                'dynamic',
                segments: [
                  PathSegment.argument('id', pattern: r'\d+'),
                ],
                children: [
                  testScreen('child3'),
                  testScreen(
                    'child4',
                    segments: [PathSegment.argument('child4')],
                  ),
                ],
              ),
            ],
    ...
    group('Dynamic and wildcard segments', () {
      test('should match screens with dynamic segments', () {
        final userProfile = testScreen(
          'userProfile',
          segments: [
            PathSegment.argument('userId', pattern: r'\d+'),
          ],
        );
      ...
      test('should handle nested screens with mixed path segments', () {
        final childScreen = testScreen(
          'child1',
          segments: [PathSegment.argument('childId', pattern: r'\d+')],
        );

        final parentScreen = testScreen(
          'parent1',
          segments: [PathSegment.static('parent')],
      ...
      test('should match nested dynamic segments with multiple patterns', () {
        final detailsScreen = testScreen(
          'details',
          segments: [PathSegment.argument('infoKey', pattern: r'[a-z]+')],
        );
        ...
        final usersScreen = testScreen(
          'users',
          segments: [PathSegment.static('users')],
    ...
    group('Screen retrieval', () {
      ...
      test('should create a registry with nested screens', () {
        final childScreen =
            testScreen('child1', segments: [PathSegment.static('child')]);
