...
import 'package:oui/src/core/router/path_match.dart';
...
void main() {
  group('PathMatch', () {
    test('rawSegments returns original segments', () {
      final segment = PathSegment.argument('userId');
      final match = PathSegmentMatch(segment: segment, original: '123');
      final pathMatch = PathMatch([testScreen('test')], [match], [], 1);

      expect(pathMatch.rawSegments, ['123']);
    });

    test('canPop returns true if screens are not empty', () {
      final pathMatch = PathMatch([testScreen('test')], [], [], 1);

      expect(pathMatch.canPop, true);
    });

    test('canPop returns false if screens are empty', () {
      const pathMatch = PathMatch([], [], [], 1);
    ...
    test('rate calculation with various segment counts', () {
      ...
      for (final testCase in testCases) {
        ...
        final matches = List.filled(
          testCase.matched,
          PathSegmentMatch(segment: segment, original: '123'),
        );
    ...
    test('rate returns the correct percentage of segments that matched', () {
      final segment = PathSegment.argument('userId');
      final match = PathSegmentMatch(segment: segment, original: '123');
      final pathMatch = PathMatch([testScreen('test')], [match], [], 2);

      expect(pathMatch.rate, 0.5);
    });

    test('rate returns 0 if expectedSegmentCount is 0', () {
      const pathMatch = PathMatch([], [], [], 0);

      expect(pathMatch.rate, 0);
    });

    test('pop removes the last count segments from the match', () {
      final segment1 = PathSegment.argument('userId1');
      final match1 = PathSegmentMatch(segment: segment1, original: '123');
      final segment2 = PathSegment.argument('userId2');
      final match2 = PathSegmentMatch(segment: segment2, original: '456');
      final pathMatch = PathMatch(
    ...
    test(
      ...
      expect(poppedMatch, PathMatch.noMatch);
    });
    ...
    test('rawSegments handles empty or invalid segments', () {
      final segment = PathSegment.argument('userId');
      final match = PathSegmentMatch(segment: segment, original: '');
      final pathMatch = PathMatch([testScreen('test')], [match], [], 1);
