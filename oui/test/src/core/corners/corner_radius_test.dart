...
void main() {
  group('CornerRadius', () {
    test('Validation for negative radius', () {
      expect(
        () => CornerRadius(radius: -1, smoothing: 0.5),
    ...
    test('Validation for smoothing greater than 1', () {
      expect(
        () => CornerRadius(radius: 1, smoothing: 1.5),
        throwsAssertionError,
      );
    });

    test('Validation for normal initialization', () {
      const radius = CornerRadius(radius: 1, smoothing: 0.5);
    ...
    test('Addition of two OuiCornerRadius objects', () {
      ...
      const radius2 = CornerRadius(radius: 2, smoothing: 0.3);
    ...
    test('Subtraction of two OuiCornerRadius objects', () {
      const radius1 = CornerRadius(radius: 3, smoothing: 0.5);
      const radius2 = CornerRadius(radius: 1, smoothing: 0.3);
      final result = radius1 - radius2;
    ...
    test('Division of a OuiCornerRadius by a scalar', () {
      const radius = CornerRadius(radius: 4, smoothing: 0.5);
      final result = radius / 2;
      expect(result.cornerRadius, 2);
    ...
    test('Integer division of a OuiCornerRadius by a scalar', () {
      const radius = CornerRadius(radius: 5, smoothing: 0.5);
      final result = radius ~/ 2;
    ...
    test('Equality comparison between two identical objects', () {
      const radius1 = CornerRadius(radius: 2, smoothing: 0.5);
      const radius2 = CornerRadius(radius: 2, smoothing: 0.5);
      expect(radius1 == radius2, true);
    });

    test('Inequality comparison between two different objects', () {
      const radius1 = CornerRadius(radius: 2, smoothing: 0.5);
      const radius2 = CornerRadius(radius: 3, smoothing: 0.5);
      expect(radius1 != radius2, true);
    });
    ...
    test('Validation of toString format', () {
      ...
      expect(
        radius.toString(),
        'CornerRadius(cornerRadius: 2.00, smoothing: 0.50)',
      );
    });

    test('Operations with zero values (radius or smoothing)', () {
      const radius = CornerRadius(radius: 0, smoothing: 0);
      final result = radius * 2;
